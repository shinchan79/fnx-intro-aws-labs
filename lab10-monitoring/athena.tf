...
locals {
  trails_bucket = aws_s3_bucket.foo.bucket
}

resource "aws_glue_catalog_table" "cloudtrail" {
  name          = "cloudtrail_logs_trails_${data.aws_caller_identity.current.account_id}_cloudtrail"
  ...
  storage_descriptor {
    ...
    location                  = "s3://${local.trails_bucket}/cloudtrail/AWSLogs/${data.aws_caller_identity.current.account_id}/CloudTrail"
    number_of_buckets         = -1
